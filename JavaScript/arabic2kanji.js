...
function k_len_three(n) {
  ...
  if (hundreds in kanji_dict) {
    return kanji_dict[hundreds] + arabic2kanji(n.slice(1));
  }
  return k_len_one(n[0]) + kanji_dict["100"] + arabic2kanji(n.slice(1));
}
...
function k_len_four(n) {
  ...
  if (thousands in kanji_dict) {
    return kanji_dict[thousands] + arabic2kanji(n.slice(1));
  }
  return k_len_one(n[0]) + kanji_dict["1000"] + arabic2kanji(n.slice(1));
}
